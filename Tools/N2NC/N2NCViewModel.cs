...
namespace krrTools.Tools.N2NC
{
    ...
    public class N2NCViewModel : ToolViewModelBase<N2NCOptions>, IPreviewOptionsProvider
    {
        ...
        // 实际值映射 - 滑条值转换为实际速度值
        private static readonly Dictionary<double, double> TransformSpeedActualDict = new Dictionary<double, double>
        {
            { 1, 0.0625 },
            { 2, 0.125 },
            { 3, 0.25 },
        ...
        public double TransformSpeedSlot
        {
            ...
            set
            {
                if (Math.Abs(_transformSpeedSlot - value) > TOLERANCE)
                {
                    ...
                    // 自动映射到实际速度值
                    if (TransformSpeedActualDict.TryGetValue(value, out double actualSpeed)) TransformSpeed = actualSpeed;
                    OnPropertyChanged();
                }
            }
        }
        ...
        public string TransformSpeedDisplay
        {
            ...
            {
                // 根据当前速度值显示对应的节拍标签
                double v = Options.TransformSpeed.Value;
