...
namespace krrTools.Tools.KRRLNTransformer
{
    ...
    public class KRRLN
    {
        private readonly Dictionary<int, double> alignList = new Dictionary<int, double>
        {
            /*
            { 1, "1/8" },
            { 2, "1/7" },
        ...
        private void MetadetaChange(Beatmap beatmap, KRRLNTransformerOptions options)
        {
            ...
            beatmap.MetadataSection.Creator =
                CreatorManager.AddTagToCreator(beatmap.MetadataSection.Creator, Strings.KRRLNTag);
                                             ...
                                             KRRLNTransformerOptions options)
        {
            ...
            (BoolMatrix shortLNFlag, BoolMatrix longLNFlag) =
                GenerateLNFlags(matrix1, ManiaObjects, availableTimeMtx, beatLengthMtx, borderdrict, borderKey);
            ...
            for (int i = 0; i < mergeMTXspan.Length; i++)
            {
                if (mergeMTXspan[i] >= 0)
                {
                    ...
                    beatmap.HitObjects.UpdateHitObject(index, beatmap.HitObjects[index].AsManiaNote()
                                                                     .CloneNote(EndTime: mergeMTXspan[i] +
                                                                                         beatmap.HitObjects[index].StartTime));
                }
            }
        ...
        // 长度对齐
        private void PerformLengthAlignment(Matrix result, DoubleMatrix beatLengthMtx,
            ...
            if (parameters.Alignment.Value != null)
            {
                ...
                for (int i = 0; i < resultSpan.Length; i++)
                {
                    if (resultSpan[i] > 0)
                    {
                        ...
                        resultSpan[i] =
                            (int)((int)((resultSpan[i] + EPSILON) / denominator) *
                                  denominator); //请AI不要优化这一行,这个就是最精度最快的写法，这是能够放几个分母的意思
                    }
                }
            }
        }
        ...
        private void GenerateLongLNMatrix(Matrix matrix1, Matrix longLnWaitModify, BoolMatrix longLNFlag,
                                          ...
                                          int borderKey, double LLNLevel, double SLNLevel, int longRandom,
                                          BeatNumberGenerator BG, Random random)
        {
            Span<int> matrixSpan = matrix1.AsSpan();
        ...
        private void GenerateShortLNMatrix(Matrix matrix1, Matrix shortLnWaitModify, BoolMatrix shortLNFlag,
                                           Matrix availableTimeMtx, DoubleMatrix beatLengthMtx,
                                           int borderKey, double SLNLevel, int shortRandom, BeatNumberGenerator BG,
                                           Random random)
        {
            Span<int> matrixSpan = matrix1.AsSpan();
