...
public class BatchProcessingOptimizationTests : IDisposable
{
    ...
    public void BatchSizeCalculation_ShouldProduceReasonableBatchCounts()
    {
        ...
        foreach (var (totalFiles, expectedBatchSize) in testCases)
        {
            var batchCount = (int)Math.Ceiling((double)totalFiles / expectedBatchSize);
            var concurrentBatches = Environment.ProcessorCount;
            ...
            // 验证并发效率（批次数/CPU核心数）
            var concurrencyRatio = (double)batchCount / concurrentBatches;
    ...
    {
        // 原始方式：每个文件一个Task
        var originalTaskCount = totalFiles;
        
        // 新的激进策略
        var batchSize = totalFiles switch
        {
            <= 80 => 1,      // 直接并发，无优化
            <= 200 => 5,     
            <= 500 => 10,    
        ...
        var optimizedTaskCount = (int)Math.Ceiling((double)totalFiles / batchSize);
        var taskReduction = (double)(originalTaskCount - optimizedTaskCount) / originalTaskCount * 100;
